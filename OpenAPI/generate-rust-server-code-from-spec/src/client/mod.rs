    ...
    C: Has<XSpanIdString>  + Clone + Send + Sync + 'static,
{
        ...
        context: &C) -> Result<GetAlbumsResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            200 => {
                ...
                let body = serde_json::from_str::<Vec<models::Album>>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {}", e)))?;


                Ok(GetAlbumsResponse::OK
                    (body)
                )
            }
            code => {
    ...
    async fn post_albums(
        &self,
        param_album: Option<models::Album>,
        context: &C) -> Result<PostAlbumsResponse, ApiError>
    {
        ...
        // Consumes basic body
        // Body parameter
        if let Some(param_album) = param_album {
        let body = serde_json::to_string(&param_album).expect("impossible to fail to serialize");
        *request.body_mut() = Body::from(body);
        ...
        match response.status().as_u16() {
            201 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::Album>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {}", e)))?;


                Ok(PostAlbumsResponse::Created
                    (body)
                )
            }
            409 => {
                ...
                let body = str::from_utf8(&body)
                    .map_err(|e| ApiError(format!("Response was not valid UTF8: {}", e)))?;
                let body = serde_json::from_str::<models::RestErrResponse>(body)
                    .map_err(|e| ApiError(format!("Response body did not match the schema: {}", e)))?;


                Ok(PostAlbumsResponse::Conflict
                    (body)
                )
            }
            code => {
        ...
        context: &C) -> Result<GetAlbumByIdResponse, ApiError>
    {
        ...
        match response.status().as_u16() {
            ...
            404 => {
                ...
                Ok(GetAlbumByIdResponse::NotFound
                    (body)
                )
            }
            code => {
