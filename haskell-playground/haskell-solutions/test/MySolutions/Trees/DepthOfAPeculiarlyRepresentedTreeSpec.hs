module MySolutions.Trees.DepthOfAPeculiarlyRepresentedTreeSpec where

import Test.Hspec ( describe, it, shouldBe, Spec )
import MySolutions.Trees.DepthOfAPeculiarlyRepresentedTree (depthOfAPeculiarlyRepresentedTree)
...
spec = do
  ...
  describe "depthOfAPeculiarlyRepresentedTree" $ do
    ...
    it "returns -1 for an EmptyTree" $
      depthOfAPeculiarlyRepresentedTree "" "0" `shouldBe` -1
