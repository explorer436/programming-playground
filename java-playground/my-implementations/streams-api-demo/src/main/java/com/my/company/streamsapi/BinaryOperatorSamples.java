 ...
 *
 * <p>So it inherits the following methods from the BiFunction Interface: 1. apply(T t, T u) 2.
 * andThen(Function<? super R, ? extends V> after)
 *
 * <p>The BinaryOperator takes two arguments of the same type and returns a result of the same type
 * of its arguments.
 *
 * <p>BinaryOperator.java @FunctionalInterface public interface BinaryOperator<T> extends
 * BiFunction<T,T,T> { } The BiFunction takes two arguments of any type, and returns a result of any
 * type.
 *
 * <p>BiFunction.java @FunctionalInterface public interface BiFunction<T, U, R> { R apply(T t, U u);
 * }
 */
public class BinaryOperatorSamples {
  ...
  public static void main(String[] args) {
    ...
    System.out.println(
        "developerWithMaxValueOfComparator : "
            + developerWithMaxValueOfComparator); // Developer{name='jaden', salary=10000}

    // We can combine the various steps into a single line - as shown below.

    // find developer with highest pay
    Developer developer =
        find(list, BinaryOperator.maxBy(Comparator.comparing(Developer::getSalary)));
    System.out.println(developer); // Developer{name='jaden', salary=10000}

    // find developer with lowest pay
    Developer developer2 =
        find(list, BinaryOperator.minBy(Comparator.comparing(Developer::getSalary)));
  ...
  // If the math operations involve primitive types like int, change to IntBinaryOperator for better
  // performance.
  ...
  /**
   * This example uses BinaryOperator and a custom Comparator to find the highest and lowest pay
   * developer from a list of developers.
   */
