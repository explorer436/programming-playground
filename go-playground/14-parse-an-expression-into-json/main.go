...
func main() {
  ...
  for _, test := range tests {
    ...
    if (actualStr != test.expected) {
      log.Error("TransformExpression(%s) = %s; expected %s", test.input, actualStr, test.expected)
    }
  }
...
func isValidCode(token string) book {
  if len(token) != 5 {
    return false
  }
...
func findLastParanthesisIndex(tokens []string, firstParanthesisIndex int) int {
  ...
  // If the closing paranthesis is not found
  return -1
}
