...
/**
 * 
	An implementation of Queue API based on the linked-list data structure is straightforward.
	It maintains the queue as a linked list 
		* in order from least recently to most recently added items, 
		* with the beginning of the queue referenced by an instance variable first
		* and the end of the queue referenced by an instance variable last. 
	To enqueue() an item, we add it to the end of the list 
	(see LinkedList.md "Building a linked list", augmented to set both first and last to refer to the new node when the list is empty).

	To dequeue() an item, we remove it from the beginning of the list 
	(using the same code as for pop() in Stack , augmented to update last when the list becomes empty). 

	The implementations of size() and isEmpty() are the same as for Stack. 

	As with Stack the implementation uses the generic type parameter Item, 
	and we omit the code to support iteration, which we consider in our Bag implementation. 
