 ...
 * Level : Respectable
 *
 * <p>Find the minimal average of any slice containing at least two elements.
 *
 * <p>A non-empty array A consisting of N integers is given. A pair of integers (P, Q), such that 0
 ...
 *
 * <p>For example, array A such that: A[0] = 4 A[1] = 2 A[2] = 2 A[3] = 5 A[4] = 1 A[5] = 5 A[6] = 8
 *
 * <p>contains the following example slices:
 *
 * <p>slice (1, 2), whose average is (2 + 2) / 2 = 2; slice (3, 4), whose average is (5 + 1) / 2 =
 * 3; slice (1, 4), whose average is (2 + 2 + 5 + 1) / 4 = 2.5.
 *
 * <p>The goal is to find the starting position of a slice whose average is minimal.
 *
 * <p>Write a function:
 *
 * <p>class Solution { public int solution(int[] A); }
 *
 * <p>that, given a non-empty array A consisting of N integers, returns the starting position of the
 * slice with the minimal average. If there is more than one slice with a minimal average, you
 * should return the smallest starting position of such a slice.
 *
 * <p>For example, given array A such that: A[0] = 4 A[1] = 2 A[2] = 2 A[3] = 5 A[4] = 1 A[5] = 5
 * A[6] = 8
 *
 * <p>the function should return 1, as explained above.
 *
 * <p>Write an efficient algorithm for the following assumptions:
 *
 * <p>N is an integer within the range [2..100,000]; each element of array A is an integer within
...
public class MinAvgTwoSlice {
  ...
  private static int incrementCountBasedOnLetterAtCurrentPostition(
