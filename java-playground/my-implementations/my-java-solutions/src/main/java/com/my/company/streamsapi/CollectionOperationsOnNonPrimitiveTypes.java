...
public class CollectionOperationsOnNonPrimitiveTypes {
    ...
    /**
     * Group members of the collection people by gender.
     */
    ...
    public Map<String, String> groupPeopleByGender_GetFullNamesGrammatically(
        ...
        Map<String, List<String>> abc = groupPeopleByGender_GetFullNames(people);

        Map<String, String> copy = new HashMap<>();
        for (Map.Entry<String, List<String>> e : abc.entrySet()) {
            copy.put(e.getKey(),
                    Joining.joinListGrammaticallyWithJava(e.getValue()));
        }
        return copy;
    }

    public Map<String, List<String>> groupPeopleByGender_GetFullNames(
            List<Person> people) {

        Map<String, List<Person>> groupedByGender = groupPeopleByGender_Collect_ListToMap_IncludeDuplicates(people);

        groupedByGender.values().forEach(l -> {
            for (Person p : l) {
        ...
        for (Map.Entry<String, List<Person>> e : groupedByGender.entrySet()) {
            copy.put(e.getKey(),
                    e.getValue().stream()
    ...
    public Map<String, String> groupPeopleByGender_GetFirstNames(
            List<Person> people) {
