...
#[structopt(
    name = "Albums API",
    version = "v1.0.0",
    about = "CLI access to Albums API"
)]
struct Cli {
...
async fn main() -> Result<()> {
    ...
    let result = match args.operation {
        Operation::GetAlbums {
            ...
            info!("Performing a GetAlbums request");

            let result = client.get_albums(
            ).await?;
            debug!("Result: {:?}", result);
        ...
        Operation::PostAlbums {
            album,
        } => {
            info!("Performing a PostAlbums request");

            let result = client.post_albums(
                album,
            ).await?;
            debug!("Result: {:?}", result);
        ...
        Operation::GetAlbumById {
            ...
            info!("Performing a GetAlbumById request on {:?}", (
                &id
            ));

            let result = client.get_album_by_id(
                id,
            ).await?;
            debug!("Result: {:?}", result);
