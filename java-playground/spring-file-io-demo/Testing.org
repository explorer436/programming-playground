...
curl --request POST   --url http://localhost:8080/api/files/upload   --header 'Content-Type: multipart/form-data'   --form file=@sample3.txt

curl --request GET --url http://localhost:8080/api/files/files
