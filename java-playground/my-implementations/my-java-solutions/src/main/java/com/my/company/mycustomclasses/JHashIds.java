...
public class JHashIds {
  ...
  public JHashIds(String salt, Integer minHashLength, String alphabet) {
    this.usedSalt =
        salt == null || salt.isEmpty() ? DEFAULT_SALT.toCharArray() : salt.toCharArray();
    ...
    prepUsedSeparators =
        consistentShuffle(removeCharFromCharArray(prepUsedSeparators, ' '), usedSalt);

    if (prepUsedSeparators.length > 0
        || prepUsedAlphabet.length / prepUsedSeparators.length > SEP_DIV_THRESHOLD) {
      int separatorsLength =
          Math.max((int) Math.ceil(prepUsedAlphabet.length / SEP_DIV_THRESHOLD), 2);

      if (separatorsLength > prepUsedSeparators.length) {
        int diff = separatorsLength - prepUsedSeparators.length;

        char[] newPrepUsedSeparators =
            Arrays.copyOf(prepUsedSeparators, prepUsedSeparators.length + diff);

        System.arraycopy(
            prepUsedAlphabet, 0, newPrepUsedSeparators, prepUsedSeparators.length, diff);
    ...
    if (prepUsedAlphabet.length < 3) {
      ...
      prepUsedSeparators =
          Arrays.copyOfRange(prepUsedSeparators, guardCount, prepUsedSeparators.length);
    } else {
