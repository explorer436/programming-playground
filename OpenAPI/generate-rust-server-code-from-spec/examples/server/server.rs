...
impl<C> Api<C> for Server<C> where C: Has<XSpanIdString> + Send + Sync
{
    ...
    {
        info!("get_albums() - X-Span-ID: {:?}", context.get().0.clone());
    ...
    {
        info!("post_albums({:?}) - X-Span-ID: {:?}", album, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
    ...
    {
        info!("get_album_by_id({}) - X-Span-ID: {:?}", id, context.get().0.clone());
        Err(ApiError("Api-Error: Operation is NOT implemented".into()))
    }
